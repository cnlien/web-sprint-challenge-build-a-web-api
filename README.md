...
- [x] Design and build endpoints for performing CRUD operations on _projects_ 
- [x] Design and build endpoints for performing CRUD operations on _actions_. When adding an action, make sure the `project_id` provided belongs to an existing `project`. If you try to add an action with an `id` of 3 and there is no project with that `id` the database will return an error.
